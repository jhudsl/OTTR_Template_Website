...
When you create a pull request, you will see something like this if everything is successful. You can click on the `preview of the website here` link to see a preview. Please note that some features may not be possible to see in the preview. For example, icons may only show up as a box.
...
```{r, fig.align='center', fig.alt= "render issue PR image", echo = FALSE, out.width="80%"}
...
For example, we can see that an R object was not found in this case in one of the files. You could identify which file by scrolling up.
